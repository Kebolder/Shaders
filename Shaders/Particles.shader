Shader "_Kebolder/Particles"
{
    Properties
    {
        [HideInInspector][KTitle(Particle Shader)] _KTitle ("", Float) = 0

        [HideInInspector][KSection(Main)] _KSectionMain ("", Float) = 0
        [KHeader(Rendering)] [Enum(Opaque,0,Cutout,1,Fade,2,Additive,3,Transparent,4,Subtract,5)] _RenderingMode ("Rendering Mode", Int) = 2
        [Enum(Normal,0,Multiply,1,Overlay,2,Subtract,3,Additive,4,Color,5,Difference,6)] _ColorMode ("Color Mode", Int) = 0
        [HideInInspector][KDivider] _KDividerMain0 ("", Float) = 0
        ...
        [KToggleSection(Emission)] _UseEmission ("Use Emission", Float) = 0
        _EmissionMap ("Emission Map", 2D) = "black" {}
        _EmissionMask ("Emission Mask", 2D) = "white" {}
        [HDR] _EmissionColor ("Emission Color", Color) = (1,1,1,1)
        _EmissionStrength ("Emission Strength", Range(0,20)) = 1.0
        [HideInInspector][KDivider] _KDividerEmission0 ("", Float) = 0
        [KHeader(Base Color Integration)] [Toggle] _EmissionBaseColorAsMap ("Use Base Colors", Float) = 0
        [Toggle] _EmissionReplaceBase ("Override Base Color", Float) = 0
        [HideInInspector][KSectionEnd] _KSectionEmissionEnd ("", Float) = 0

        [KToggleSection(AudioLink (VRChat))] _UseAudioLink ("Use AudioLink", Float) = 0
        [KSubHeader(Global Settings)] _AudioLinkSmoothing ("Smoothing", Range(0,5)) = 0.0
        [HideInInspector][KDivider] _KDividerAudioLink0 ("", Float) = 0
        [KSubHeader(Emission Intensity Modulation)] [Toggle] _AudioLinkStrength ("Enable Strength Modulation", Float) = 1
        ...
        [HideInInspector][KDivider] _KDividerAudioLink1 ("", Float) = 0
        [KSubHeader(Color Shifting)] [Toggle] _AudioLinkColorShift ("Enable Color Shifting", Float) = 0
        ...
        [HideInInspector][KDivider] _KDividerAudioLink2 ("", Float) = 0
        [KSubHeader(Size Modulation)] [Toggle] _AudioLinkSize ("Enable Size Modulation", Float) = 0
        ...
        [HideInInspector][KSectionEnd] _KSectionAudioLinkEnd ("", Float) = 0

        [KToggleSection(Dissolve Effects)] _UseDissolve ("Use Dissolve Effect", Float) = 0
        ...
        [HideInInspector][KSectionEnd] _KSectionDissolveEnd ("", Float) = 0

        [KToggleSection(Edge Fade)] _UseEdgeFade ("Use Edge Fade", Float) = 0
        _EdgeFadeDistance ("Edge Fade Distance", Range(0,0.5)) = 0.1
        [HideInInspector][KSectionEnd] _KSectionEdgeFadeEnd ("", Float) = 0

        [KToggleSection(Distortion Effects)] _UseDistortion ("Use Distortion", Float) = 0
        _DistortionMap ("Distortion Normal Map", 2D) = "bump" {}
        _DistortionStrength ("Distortion Strength", Range(0,1)) = 0.1
        [HideInInspector][KDivider] _KDividerDistortion0 ("", Float) = 0
        [KSubHeader(Distortion Animation)] _DistortionScrollX ("Distortion Scroll X", Float) = 0.0
        _DistortionScrollY ("Distortion Scroll Y", Float) = 0.0
        _DistortionScale ("Distortion UV Scale", Float) = 1.0
        [HideInInspector][KSectionEnd] _KSectionDistortionEnd ("", Float) = 0

        [HideInInspector][KSection(Settings)] _KSectionSettings ("", Float) = 0
        [KSubHeader(Soft Particles)] _InvFade ("Soft Particles Factor", Range(0.01,3.0)) = 1.0
        [HideInInspector][KDivider] _KDividerSettings0 ("", Float) = 0
        [KSubHeader(Dual Sided Rendering)] [Toggle] _DualSided ("Dual Sided Rendering", Float) = 0
        ...
        Pass
        {
            ...
            float GetAudioReactiveValue(int band)
            {
                ...
                else
                {
                    ...
                    float smoothPower = 1.0 + (_AudioLinkSmoothing * 0.4);
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float2 mainUV = KebolderUV(baseUV, _MainTex_ST);
                if (_UseDistortion > 0.5)
                {
                    float2 distortionUV = KebolderUV(baseUV, _DistortionMap_ST) * _DistortionScale;
                ...
                if (_UseEmission > 0.5)
                {
                    half4 emissionMap = tex2D(_EmissionMap, KebolderUV(baseUV, _EmissionMap_ST));
                    half4 emissionMask = tex2D(_EmissionMask, KebolderUV(baseUV, _EmissionMask_ST));
                ...
                if (_UseDissolve > 0.5)
                {
                    float2 dissolveUV = KebolderUV(baseUV, _DissolveTexture_ST) * _DissolveScale;
