...
// Features panosphere effect based on camera position for panoramic sphere texture mapping
Shader "_Jax/Particles"
{
    Properties
    {
        ...
        [Toggle] _UsePlaid ("Use Unmoving Plaid", Float) = 0
        _PlaidTex ("Plaid Texture", 2D) = "white" {}
        _PlaidScale ("Plaid Screen Scale", Float) = 1.0
        _PlaidStrength ("Plaid Blend Strength", Range(0,1)) = 1.0
        _PlaidMask ("Plaid Mask", 2D) = "white" {}
        ...
        [Enum(Off,0,Back,2,Front,1)] _Cull ("Culling Mode", Float) = 2
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _UsePlaid;
            sampler2D _PlaidTex;
            sampler2D _PlaidMask;
            float4 _PlaidMask_ST;
            float _PlaidScale;
            float _PlaidStrength;
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // Apply tiling and offset using Unity's standard macro
                float2 mainUV = TRANSFORM_TEX(i.texcoord, _MainTex);
                ...
                half baseAlpha = col.a;

                // Emission
                half3 emission = half3(0,0,0);
                if (_UseEmission > 0.5)
                {
                    // Apply tiling and offset for emission map
                    float2 emissionUV = TRANSFORM_TEX(i.texcoord, _EmissionMap);

                    half4 emissionMap = tex2D(_EmissionMap, emissionUV);
                    half4 emissionMask = tex2D(_EmissionMask, emissionUV);
                ...
                if (_UsePlaid > 0.5)
                {
                    float2 plaidUV = (i.screenPos.xy / i.screenPos.w) * _PlaidScale;
                    half4 plaidSample = tex2D(_PlaidTex, plaidUV);
                    half3 plaidColor = plaidSample.rgb * i.color.rgb;
                    float2 maskUV = TRANSFORM_TEX(i.texcoord, _PlaidMask);
                    half plaidMask = tex2D(_PlaidMask, maskUV).r;
                    half plaidBlend = saturate(_PlaidStrength * plaidMask);
                    col.rgb = lerp(col.rgb, plaidColor, plaidBlend);
                }
                col.a *= _Alpha;
